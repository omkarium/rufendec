...
use aes_gcm::aes::cipher::{
    ...
    typenum::{/*U32, U16*/ U12, B0, B1, UInt, UTerm}
};
...
#[derive(
    clap::ValueEnum, Clone, Debug, Copy
)]
pub enum Mode{
    ECB,
    GCM
...
pub fn encrypt_files(file_list: Vec<PathBuf>, thread_count: usize, source_dir_name: &str, target_dir_name: &str, mode: Mode) {
               ...
               s.spawn(move |_| {
                   if let Ok(file_data)  = fs::read(file.clone()){
                     match mode {
                        ...
                        Mode::GCM => {
                            let cipher: aes_gcm::AesGcm<aes_gcm::aes::Aes256, _, _> = Aes256Gcm::new(&GCM_32BYTE_KEY.lock().unwrap().as_slice()[0]);
                            let nonce = Aes256Gcm::generate_nonce(&mut OsRng); // 96-bits; unique per message
                            match cipher.encrypt(&nonce, file_data.as_ref()) {
                                Ok(encrypted_bytes) => {
                                    let new_file_name = file.as_os_str().to_str().expect("Found a bad file").replace(source_dir_name, target_dir_name).to_string() + ".enc";
                                    println!("Encrypted file :: {}", new_file_name);
                                    let _ = fs::write(new_file_name, [encrypted_bytes, nonce.to_vec()].concat());
...
pub fn decrypt_files(file_list: Vec<PathBuf>, thread_count: usize, source_dir_name: &str, target_dir_name: &str, mode: Mode) {
   let pool = rayon::ThreadPoolBuilder::new().num_threads(thread_count).build().unwrap();
   pool.install(|| {
       rayon::scope(|s| {
           for file in file_list {
